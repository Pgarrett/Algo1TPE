...
\asegura[siNoVuelanNoSeModifican]{(\forall d \selec enjambreDrones(pre(s))) \\ \neg enVuelo(d) \Rightarrow droneEnLista(enjambreDrones(s), d)}

\asegura[siVuelanFertilizan]{(\forall d \selec enjambreDrones(pre(s))) \\ enVuelo(d) \Rightarrow droneEnLista(enjambreDrones(s), droneDespuesDeFertilizar(d))}

\end{problema}

%revisar si se incluye la parcela actual%
...
\aux{droneDespuesDeFertilizar}{s: Sistema, d: Drone}{d: Drone}{TO-DO}
