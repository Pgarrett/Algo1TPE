...
\asegura{(\forall {p} \selec {parcelasConCultivo(s)}) estadoDelCultivo(s, prm(p), sgd(p)) == estadoDelCultivo(pre(s), prm(p), sgd(p))}
\asegura{(\forall {d} \selec {enjambreDrones(pre(s))}) bateria(d) \geq b \Rightarrow droneEnLista(enjambreDrones(s), d)}

\asegura{(\forall {d} \selec {enjambreDrones(pre(s))}) bateria(d) < b \Rightarrow (\exists d' \selec enjambreDrones(s)) id(d') == id(d) \land bateria(d') == 100 \land \neg enVuelo(d') \land posicionActual(d') == posicionGranero(campo(s)) \land (mismos(productosDisponibles(d),  productosDisponibles(d')))}
...
%TODO!%
\noindent \aux{posicionGranero}{c: Campo}{(\ent, \ent)}{}
